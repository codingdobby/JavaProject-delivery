...
import JaBae.Gisa.Event.MyDeliveryEventClass;
...
public class MyDeliveryList extends JFrame {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					MyDeliveryList frame = new MyDeliveryList(gisaId);
	 ...
	 * @throws SQLException 
	 */
	public MyDeliveryList(String gisaId) throws SQLException {
		this.gisaId = gisaId;
		setTitle("자배 택배");
		...
		JLabel lblTitle = new JLabel("배송목록");
		...
		titleDList.add("송장 번호"); titleDList.add("물품 명"); titleDList.add("이름");
		titleDList.add("배송 주소"); titleDList.add("전화 번호");
		...
		JButton btnAccept = new JButton("배송완료");
		...
		for(GisaVO getDList : dList) {  //리스트 불러옴
			modelDList.addRow(new Object[] { getDList.getOrderNo(), getDList.getObjectName(), 
					getDList.getNameOrderer(), getDList.getAddrOrderer(), getDList.getTelOrderer()});
		}
		
		
		/*****************************************ActionListener******/
		btnAccept.addActionListener(new MyDeliveryEventClass(tableDList, gisaId, modelDList));
